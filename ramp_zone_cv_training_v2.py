...
class _ContextRampDataset(Dataset):
    ...
    def __init__(
        ...
        x, x_feat, y = _df_to_arrays(self.frame, z_cols, gap_cols, pad_cols, feature_cols)
        self.x = x.copy()
...
class _StationEncoder(nn.Module):
    def __init__(
        self,
        in_channels: int = 3,
        num_features: int = 2,
        ...
        if self.use_scalar_features:
            ...
            local_dim = 64
        self.station_embedding = nn.Sequential(
            nn.Linear(local_dim, embedding_dim),
            nn.LeakyReLU(0.1),
            nn.Dropout(dropout),
        )
...
class _RampCNN(nn.Module):
    def __init__(
        ...
        self.station_encoder = _StationEncoder(
        ...
        self.context_encoder = nn.Sequential(
            nn.Conv1d(embedding_dim + 1, embedding_dim, kernel_size=3, padding=1),
    ...
    def forward(self, x_sequence: torch.Tensor, feature_sequence: torch.Tensor, context_mask: torch.Tensor) -> torch.Tensor:
        b_size, t_size, channels, length = x_sequence.shape
        x_flat = x_sequence.reshape(b_size * t_size, channels, length)
        feature_flat = feature_sequence.reshape(b_size * t_size, -1)
        embedding_flat = self.station_encoder(x_flat, feature_flat)
        embedding_sequence = embedding_flat.reshape(b_size, t_size, self.embedding_dim)
        ...
        center_embedding = combined_embeddings[:, t_size // 2, :]
...
def _prepare_objects(train_frame, val_frame, cfg, z_cols, gap_cols, pad_cols, seed):
    ...
    train_final = capped_train.sample(frac=1.0, random_state=seed).reset_index(drop=True)
    train_final["is_augmented"] = 0
    train_final["aug_parent_id"] = -1
    if val_frame is not None:
    ...
    train_ds = _ContextRampDataset(
        train_final,
        z_cols,
        gap_cols,
        pad_cols,
        cfg["ACTIVE_FEATURE_COLS"],
        z_mean,
        z_std,
        feat_mean,
        feat_std,
        cfg["CONTEXT_WINDOW_SIZE"],
        cfg["MAX_CONTEXT_GAP_M"],
    )
...
def _train_one_model(train_frame, val_frame, fixed_epochs, fold_name, cfg, z_cols, gap_cols, pad_cols, seed):
    ...
    model = _RampCNN(
        ...
        embedding_dim=cfg["STATION_EMBEDDING_DIM"],
...
def _predict_frame(model, frame, prepared, cfg, z_cols, gap_cols, pad_cols, split_name, holdout_zone=None):
    ...
    with torch.no_grad():
        for xb, xb_feat, xb_mask, _ in eval_loader:
            logits_parts.append(model(xb.to(cfg["device"]), xb_feat.to(cfg["device"]), xb_mask.to(cfg["device"])).cpu().numpy())
...
def _save_confusion_matrices(pred_df: pd.DataFrame, output_dir: Path, cfg: dict) -> tuple[Path, Path]:
    ...
    ax.set_title("V2 Zone CV Held-Out Confusion Matrix")
    ...
    ax_norm.set_title("V2 Zone CV Held-Out Confusion Matrix (Normalized)")
...
def run_zone_holdout_cv_v2(ns: dict) -> dict:
    cfg = {name: ns[name] for name in [
        ...
        "TRAIN_FINAL_MODEL_AFTER_CV", "FINAL_EPOCH_REDUCTION", "USE_CONTEXT_MODEL", "CONTEXT_WINDOW_SIZE",
        "CONTEXT_HALF_WIDTH", "STATION_EMBEDDING_DIM", "MAX_CONTEXT_GAP_M", "USE_SPATIAL_ATTENTION", "device",
    ]}
    if cfg["ENABLE_RAMP_AUG"]:
        raise ValueError("Disable ENABLE_RAMP_AUG for contextual zone CV; row-level augmentation would disturb context windows.")
    if cfg["CONTEXT_WINDOW_SIZE"] % 2 != 1:
        raise ValueError("CONTEXT_WINDOW_SIZE must be odd so there is one centre station.")
    ...
    if missing:
        raise ValueError(f"Missing required columns for v2 zone CV: {missing[:10]}{'...' if len(missing) > 10 else ''}")
    ...
    print(f"V2 zone-wise CV output dir: {output_dir}")
    ...
    print(f"Context window: {cfg['CONTEXT_WINDOW_SIZE']} | max_gap={cfg['MAX_CONTEXT_GAP_M']}")
    print("Rows by zone:")
    ...
    for fold_idx, holdout_zone in enumerate(holdout_zones, start=1):
        print(f"\n--- V2 zone CV fold {fold_idx}/{len(holdout_zones)} | holdout zone {holdout_zone} ---")
        ...
        model, prepared, history, fit_info = _train_one_model(train_fold, val_fold, None, f"v2_zone{holdout_zone}", cfg, z_cols, gap_cols, pad_cols, cfg["RANDOM_SEED"])
        ...
        row = {
            ...
            "train_context_n": int(len(prepared["train_ds"])),
            "val_context_n": int(len(prepared["val_ds"])),
    ...
    print("\n=== V2 zone-wise CV aggregate held-out metrics ===")
    print(f"Held-out context rows: {len(heldout_pred):,}")
    ...
    print("Fold summary:")
    print(summary_df[["holdout_zone", "train_n", "val_n", "train_context_n", "val_context_n", "best_epoch", "accuracy", "macro_f1_present_classes", "macro_f1_all_classes", "present_classes"]].to_string(index=False))
    ...
    if cfg["TRAIN_FINAL_MODEL_AFTER_CV"]:
        print(f"\n=== Training final V2 model on all zones for {final_epochs} epochs ===")
        final_model, final_prepared, final_history, _ = _train_one_model(df, None, final_epochs, "v2_final_all_zones", cfg, z_cols, gap_cols, pad_cols, cfg["RANDOM_SEED"])
        ...
        torch.save({
            ...
            "context_half_width": cfg["CONTEXT_HALF_WIDTH"],
            "station_embedding_dim": cfg["STATION_EMBEDDING_DIM"],
            "max_context_gap_m": cfg["MAX_CONTEXT_GAP_M"],
            "use_spatial_attention": cfg["USE_SPATIAL_ATTENTION"],
        }, model_path)
        meta = {
            "model_type": "contextual_station_embedding_cnn",
            ...
            "best_val_loss": None,
            "best_val_f1": aggregate_metrics["macro_f1_present_classes"],
            "validation_accuracy": aggregate_metrics["accuracy"],
            "validation_macro_f1": aggregate_metrics["macro_f1_present_classes"],
        ...
        print(f"Saved final V2 CV-selected model -> {model_path}")
        print(f"Saved final V2 model metadata    -> {meta_path}")
        result.update({
    ...
    print(f"Saved V2 CV fold summary         -> {summary_path}")
    print(f"Saved V2 CV aggregate summary    -> {aggregate_path}")
    print(f"Saved V2 CV held-out predictions -> {pred_path}")
    print(f"Saved V2 CV confusion matrix     -> {cm_path}")
    print(f"Saved V2 CV normalized matrix    -> {cm_norm_path}")
    return result
